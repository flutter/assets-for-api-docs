...
class _DiagramFlutterBinding extends BindingBase
  ...
  Future<image.Image> takeSnapshot() async {
...
class DiagramController {
  ...
  Future<File> drawDiagramToFile(File outputFile) async {
  ...
  Future<List<image.Image>> drawAnimatedDiagramToImages({
    ...
    Duration now = start;
    List<image.Image> outputImages = <image.Image>[];
    while (now <= end) {
      image.Image captured = await drawDiagramToImage();
      print('Generated frame for $now, ${captured.length} bytes, ${captured.width}x${captured.height}');
  ...
  Future<List<File>> drawAnimatedDiagramToFiles({
    ...
    while (now <= end) {
      ...
      image.Image captured = await drawDiagramToImage();
      List<int> encoded = image.encodeNamedImage(captured, outputFile.path);
      print('Writing frame $index ($now), ${encoded.length} bytes, ${captured.width}x${captured.height}, to ${outputFile.absolute.path}');
